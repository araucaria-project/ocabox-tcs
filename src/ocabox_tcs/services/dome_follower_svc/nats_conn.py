...
class NatsConn:
    ...
    async def connect(self) -> None:
        ...
        if not self.messenger.is_open:
            # self.manager.svc_logger.info(f'Nats not opened, connecting...')
            # host = '192.168.8.140' #TODO take from config
            # port = 4222 #TODO take from config
            # self.manager.svc_logger.info(f'Trying connect to nats: {host}:{port}')
            # await self.messenger.open(host=host, port=port)
            # self.manager.svc_logger.info(f'Nats connected to {host}:{port}')
            # self.connected = True
            # self.messenger_self_managed = True
            self.loger.error('Messenger/NATS not ready!')
            exit(1)
        else:
            self.manager.svc_logger.info(f'Nats already connected')
            self.connected = True

    async def rpc_follow_on(self, rpc: Rpc) -> None:
        self.manager.svc_logger.info(f'Follow on rpc request received')
        ...
        self.manager.svc_logger.info(f'Sent rpc response: follow on')

    async def rpc_follow_off(self, rpc: Rpc) -> None:
        self.manager.svc_logger.info(f'Follow off rpc request received')
        ...
        await rpc.response_now(data=data, meta=meta)
        self.manager.svc_logger.info(f'Sent rpc response: follow off')

    async def rpc_state(self, rpc: Rpc) -> None:
        self.manager.svc_logger.info(f'State rpc request received')
        data = {
        ...
        await rpc.response_now(data=data, meta=meta)
        self.manager.svc_logger.info(f'Sent rpc response: status')
    ...
    async def start_responders(self) -> None:
        ...
        self.manager.svc_logger.info(f"All responders started")
    ...
    async def msg_rpc_responder(self, subject: str, callb: Callable):
        ...
        self.manager.svc_logger.info(f'Rpc responder for {subject} started')

    async def close(self) -> None:
        if self.connected and self.messenger_self_managed:
            await self.messenger.close()
            self.connected = False
            self.manager.svc_logger.info(f'Nats disconnected.')

    async def get_obs_cfg(self) -> Dict:
        _obs_cfg, meta = await single_read('tic.config.observatory', wait=10)
        self.manager.svc_logger.info(f'Obtained Observatory Config. Published: {_obs_cfg["published"]}')
        return _obs_cfg
