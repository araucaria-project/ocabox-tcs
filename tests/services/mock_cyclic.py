...
class MockCyclicService(BasePermanentService):
    ...
    async def stop_service(self):
        ...
        if self.cycle_task:
            ...
            try:
                ...
                pass
        self.logger.info(f"Stopped after {self.cycle_count} cycles")
        ...
        try:
            while self.is_running:
                ...
                try:
                    ...
                    if self.config.fail_on_cycle > 0 and self.cycle_count == self.config.fail_on_cycle:
            ...
            self.logger.debug("Cycle loop cancelled")
            raise
        except Exception as e:
            self.logger.error(f"Fatal error in cycle loop: {e}")
            raise
    ...
    async def _execute_cycle(self):
        ...
        if self.config.fail_on_cycle > 0 and self.cycle_count == self.config.fail_on_cycle:
            self.logger.error(f"Simulated failure on cycle {self.cycle_count}")
            raise RuntimeError(f"Simulated failure on cycle {self.cycle_count}")

        # Simulate work
        await asyncio.sleep(self.config.execution_duration)
